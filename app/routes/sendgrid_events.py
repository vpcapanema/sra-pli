...
def _check_sendgrid_token(
    ...
    if not expected:
        raise HTTPException(503, detail="SENDGRID_EVENT_WEBHOOK_TOKEN não configurado.")
...
def _buscar_notificacao(db: Session, evento: dict[str, Any]) -> NotificacaoEnvio | None:
    ...
    return next(
        (item for item in rows if raw.startswith(str(item.sendgrid_message_id) + ".")),
        None,
    )
...
async def sendgrid_events(
    ...
    _token: None = Depends(_check_sendgrid_token),
):
    eventos = await request.json()
    if not isinstance(eventos, list):
        raise HTTPException(400, detail="Payload do SendGrid deve ser uma lista.")
    aplicados = 0
    ignorados = 0
