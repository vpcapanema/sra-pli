...
from ..notificacoes.email_sender import preview_assunto_notificacao, preview_corpo_notificacao
...
def _contexto_preview_email_fake(user: Any) -> dict[str, Any]:
    ...
    A árvore de modelos replica o sumário canônico ``SECOES_PADRAO`` (igual à
    página de modelos Word). Para dados de um relatório real no BD, use
    ``?relatorio_id=`` na rota de pré-visualização.
    """
...
def contexto_email_sumario_padrao(nome_destinatario: str = "Autor exemplo") -> dict[str, Any]:
    """Contexto de e-mail com árvore ``SECOES_PADRAO`` completa (scripts / ficheiros em ``tmp/``)."""
...
def _preview_email_query(*, raw: str, tipo: str, relatorio_id: int | None) -> str:
...
def _iframe_src_data_html(email_html: str) -> str:
    """Atributo ``src`` seguro para iframe com documento HTML embutido (base64)."""
...
def _html_preview_shell(assunto: str, tipo: str, raw_url: str, html_body: str) -> str:
    """Página com barra + iframe; o corpo vai em data: URL para não depender de 2º GET.

    Um segundo GET para ``?raw=1`` dentro do iframe costuma chegar **sem** o cookie
    de sessão (``SameSite=lax`` em subframe), o que devolvia a tela de login em branco.
    """
...
def _fragmento_preview_email_por_tipo(
    ...
    """Índice (link) e HTML de uma seção de pré-visualização para ``tipo``."""
...
def _html_preview_todas_mensagens(
    ...
    """Uma página com as três mensagens do ciclo (mesmo HTML do envio por tipo)."""
...
def dev_preview_email_notificacao(
    ...
    *,
    ...
    """Renderiza o mesmo HTML/texto do envio (SendGrid), para revisar o modelo.

    Sem ``raw=1``: página com barra (assunto + tipo) e iframe do corpo.

    Com ``raw=1``: somente o HTML do e-mail (como recebido na caixa).

    Query:

    - ``tipo``: ``abertura`` | ``lembrete`` | ``ultima_chamada``.
    - ``relatorio_id`` (opcional): monta links e dados a partir do relatório real;
      se ausente ou inválido, usa exemplos fixos com ``APP_BASE_URL``.
    """
    if not modais_preview_allowed():
        return HTMLResponse(
...
def dev_preview_emails_notificacao_todas(
    ...
    *,
    ...
    """Lista as três mensagens do ciclo (abertura, lembrete, última chamada) na mesma página.

    Usa o mesmo ``preview_corpo_notificacao`` do envio real — é o conteúdo consolidado
    em ``email_notificacao.html``, variando só o parágrafo inicial conforme ``tipo``.
    """
    if not modais_preview_allowed():
        return HTMLResponse(
            content=_PREVIEW_EMAIL_DESLIGADO_HTML,
